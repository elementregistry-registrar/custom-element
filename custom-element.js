...
function parse(html,config) {
	...
	if((text[0]==="{" || text[0]==="[") || config.type==="application/json")  {
		...
		try {
			script = config.script = Function("return async function(target) { " + text + "}")();
		} catch(error) {
...
function getConfig(template,el) {
	let target,
		...
		parameterel = parametersource==="" ? el.previousElementSibling : (parametersource!==null ? document.getElementById(parametersource) : null);
	if(parametersource!==null && parametersource.length>0 && !parameterel) {
	...
	parse(html,config);
	if(eattributes.target) {
		target = document.getElementById(eattributes.target);
		if(!target) {
			throw new Error(`Unable to find target #${target} for ${tagname}`)
		}
	} else {
		target = document.createElement(eattributes["target-tagname"]||"div");
		target.id = (Math.random()+"").substring(2);
	}
	if(!eattributes.target) {
		el.after(target);
	}
					...
					constructor(model={}) {
						...
						if(config.previous) {
							if(config.attributes.hidden) {
								config.previous.setAttribute("hidden","");
						...
						if(config.isolate && globalThis) {
							...
							iframe.srcdoc = `${styles ? styles.outerHTML : ""}` +
								...
								`${config.target ? `<div id="${config.target.id}"></div>` : ''}` +
						...
						if(config.attributes.editable) {
							...
							editor.className = "downrunner-editor";
						...
						const parameters  = {attributes:config.attributes},
							run = (text=editor ? editor.value : config.text) => {
								...
								if(clone.content.lastElementChild.tagName==="SCRIPT" && (!clone.content.lastElementChild.hasAttribute("type") || ["text/javascript","application/javascript","javascript"].includes(clone.content.lastElementChild.getAttribute("type")))) {
									const f = new Function("return async function(globalThis,model,RJSON,require,document,parameters,target) { with(model) { " + clone.content.lastElementChild.innerText + " } }")();
									...
									if(config.error) {
										...
										f.call(this,null,this.model,RJSON,require,document,parameters,config.target);
									}
								}
								if(editor) {
									setInterval(() => {
										const lines = (editor.value.match(/.*\n/g)||[]),
											fontsize = getFontWidth(config.target),
											maxwidth = lines.reduce((accum,line) => accum = Math.max(accum,line.length),80) * fontsize,
											width = maxwidth > parseInt(config.target.scrollWidth) ? 80 * fontsize : config.target.scrollWidth;
										editor.style.width=Math.min(window.innerWidth-(20+fontsize),width);
									},500)
								}
							};
						this.run - run;
